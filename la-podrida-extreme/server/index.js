...
io.on('connection', (socket) => {
  ...
  socket.on('place_bid', ({ roomId, bidAmount }) => {
      ...
      if (currentIndex === activeOrder.length - 1) {
         const previousSum = state.players.reduce((sum, p) => {
             return p.id !== state.players[pIndex].id ? sum + (p.bid || 0) : sum;
         }, 0);
         const cardsDealt = ROUND_SEQUENCE[state.roundIndex];
         const blocked = cardsDealt - previousSum;
         
         if (cardsDealt !== 1 && blocked >= 0 && bidAmount === blocked) {
             state.players[pIndex].bid = null;
             return; 
         }
